...
- `src/components/AdminPanel.tsx` — Add, archive, restore, rescan, Make legend with an inline note plus Back to the board, two step confirm permanent remove, and a name or @handle search on Friends on the board, plus board settings for visible board columns, the Convex mentions tab toggle, rank badges, and the Slack digest, with links to the groups and branding pages.
- `src/components/GroupsPanel.tsx` — Custom groups admin: create, rename, describe, reorder, show or hide, the internal (admins only) toggle, per group board column toggles with a defaults reset, two step delete, member roster with name or @handle search, add by handle, per member mute, per member Make legend and Back to the board, and two step remove, and X List import with saved list re-sync.
