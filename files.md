...
- `src/components/GroupsPanel.tsx` — Custom groups admin: create, rename, describe, reorder, show or hide, the internal (admins only) toggle, per group board column toggles with a defaults reset, two step delete, member roster with add by handle and two step remove, and X List import with saved list re-sync.
