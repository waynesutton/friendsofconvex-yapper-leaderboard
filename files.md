...
- `src/components/AdminPanel.tsx` — Add, archive, restore, rescan, retire with an inline note and unretire, and two step confirm permanent remove controls plus board settings for visible board columns, the Convex mentions tab toggle, rank badges, and the Slack digest, with links to the groups and branding pages.
- `src/components/GroupsPanel.tsx` — Custom groups admin: create, rename, describe, reorder, show or hide, the internal (admins only) toggle, per group board column toggles with a defaults reset, two step delete, member roster with add by handle, per member mute, and two step remove, and X List import with saved list re-sync.
