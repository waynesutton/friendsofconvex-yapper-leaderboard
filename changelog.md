...
- Search on the admin people lists. Friends on the board (`/admin`) and each group member roster (`/admin/groups`) now filter by display name or @handle, same rule as the public board. The heading shows N of M matches, a miss reads "No yappers match that search.", and Sync everyone still refreshes the whole board. The public leaderboard search was already there and still filters whichever pill is active, including group boards. PRD: prds/admin-yapper-search.md (2026-08-31).

- Retire is now reachable from the group roster. Every member row in `/admin/groups` has the same Retire control as the main `/admin` list: an inline note field, Retire and publish, then a Champion page link and Unretire once they are retired. Before this an admin working inside a group could only Mute, and had to go find the person on the main board to retire them. Retiring from a group is still profile wide, so they leave the main board and every other group too; the panel copy and `/admin/docs` say so, and Mute turns off on a retired row since they are already off every board (2026-08-31).

- Group boards can mute a member. A Mute button on every member row in `/admin/groups` keeps that person in the group and in the public list while taking their rank away: they render below an "In the group, off the ranking" divider with a dash instead of a number and no medal, and column sorts and search cannot pull them back above it. Mute lives on the membership, so the same person stays ranked on every other board, and they still count toward the group's member count and pill. PRD: prds/group-mute-and-retire-mode.md (2026-08-31).
- Retire mode for undefeated champions. A Retire button on each `/admin` row opens an inline note field ("why are we retiring them"), and confirming pulls that person out of every ranking (Yappers, Convex mentions, and group boards) and publishes a champion page at `/retired/<handle>`: racing stripe card, the admin's note, their final posts, engagements, impressions, and followers, and a Post on X button. Crawlers get per person title and description tags on that URL. Unretire puts them straight back in the running and closes the page. Different from Archive, which hides someone entirely; retired people keep syncing and keep their history. PRD: prds/group-mute-and-retire-mode.md (2026-08-31).
- Forks can hide the main Yappers list. Board settings has a "Show the main Yappers tab" toggle beside the Convex mentions one; turning it off removes the pill and the public board opens on the first remaining pill (Convex mentions or the first custom group). Direct ?board=impressions links fall back the same way. Hiding both built-in tabs requires at least one visible custom group, so the board can never go blank. Missing settings default to visible, so nothing changes on deploy. PRD: prds/hide-main-yappers-tab.md (2026-08-16).
...
- Retire mode is now called Legends. "Retired" read like someone quit, and this is the happiest thing that can happen on the board. The admin control is Make legend, undoing it is Back to the board, the page lives at `/legends/<handle>` with the headline "Undefeated legend", and the database fields are `legendAt` and `legendNote`. Links already shared from `/retired/<handle>` still resolve: crawlers get the same per person card and the app redirects to the new URL. Deployments with pre-rename records deploy first, then run `npx convex run migrations:backfillLegendFields` once; the backfill cannot run before the deploy ships it (2026-09-01).
- The legend page shows two numbers instead of four. Posts and followers stay; engagements and impressions are gone, since those are the metrics the race is fought over and this page is the opposite of a race. The public query stopped returning them entirely, so the payload carries less than it did (2026-09-01).
...
- Removing someone from a group now asks for confirmation. The first click arms the button as Confirm remove with a note naming the handle; the second click removes them from the group (they stay on the main board). Before this one stray click dropped a member instantly (2026-08-16).
- The Import members from an X List button no longer stretches across the group card. The button shared the add-member input's three column grid, and without the @ prefix the input collapsed while the button took the whole flexible column. The input now sits alone with the sync button on its own row below, and the helper text spells out that the import calls the X API with the X_BEARER_TOKEN deployment variable and re-syncs from the saved list id (2026-08-16).
- The daily sync no longer skips the top of the board. It selected targets from the impressions index in default ascending order with a 100 row cap, so a board past 100 people refreshed its 100 lowest-impression profiles and left the highest ones stale until an admin refreshed them by hand. Sync targets now page every active profile in join order through a cursor, in batches, and a run that nears the action time limit schedules a continuation that picks up from the cursor until everyone has been attempted once. The admin Sync all message says when a remainder is finishing in the background. Refs [issue 2](https://github.com/waynesutton/friendsofconvex-yapper-leaderboard/issues/2) (2026-08-16).
...
- The hero header no longer shows the "Friends of Convex · people edition / Top signal / 7 days" eyebrow line above the headline. The markup is commented out in the board component, not deleted, so it can return with one edit (2026-08-16).
- The board hero header dropped its radial gradient glow. The Convex theme painted a soft red radial behind the headline, a pattern that reads as AI generated design, so the header now sits on its flat dark base with the racing lines image alone. Radial gradient backgrounds are banned across the app going forward; header washes stay flat or linear (2026-08-16).
- Posts now count replies, matching the posts number in X analytics. A post is an original post, a quote post, or a reply from the last seven days; only reposts are excluded. The Convex mention scan reads the same widened set and now also matches a long post's full text and expanded links that point to convex.dev, so a reply about Convex or a `t.co` wrapped convex.dev link counts. Tooltips, the About page, and the README state the new rules; the private engagements X analytics shows (link clicks, profile visits, detail expands) remain out of reach of the public API and the copy says so. Numbers move for everyone on the next refresh. Refs [issue 2](https://github.com/waynesutton/friendsofconvex-yapper-leaderboard/issues/2) (2026-08-16).
- Every public description of the ranking now says public engagement, which is what the homepage actually ranks by: the screen reader board heading, the About methodology, the README, the `llms.txt` site description, and the generated `sitemap.md`, whose people list is now sorted in the board's canonical order (synced rows first, then engagements, impressions, posts, and join date) instead of raw impression index order (2026-08-16).
- The About page and README now say every active profile is refreshed in the same daily run, in join order, however large the board gets. The README also fixes the cron time (8:17 AM Pacific, not 8), documents `npm run test`, and notes that `npm run check` runs the tests (2026-08-16).
- The Impressions column on the Yappers board now renders like Posts and Engagements: same size, weight, and ink. It was a `<strong>` element with a 15px override, which made it read as the ranking metric when the board actually ranks by engagement (2026-08-16).
- The Yappers / Convex mentions control is now a two lane channel switch: cream sheet track, full ink border, equal clickable lanes, a sliding ink thumb, and a coral live pip on the selected ranking so it no longer reads as a static label. Hover fills the rest lane. Arrow keys move between the two. Import source tabs keep the old inset style (2026-08-16).
...
- Internal admin only boards. Any custom group can be marked internal from `/admin/groups`: its pill then renders only for signed in admins (with a lock icon), everyone else is blocked server side (a shared `/?board=` link falls back to Yappers and the leaderboard query returns nothing for a guessed group id), and the board stays out of `llms.txt`, `sitemap.md`, and `sitemap.xml`. Good for an internal team board that admins want to watch without publishing. Existing groups stay public with no migration (2026-08-17).
- Custom groups on the board. Admins can spotlight any circle (a team, a conference, a cohort) as its own leaderboard pill from the new `/admin/groups` page: create, rename, describe, reorder, show or hide, and delete groups, add members by X handle, or paste a public X List URL and import up to 100 members in one click (the list id is saved for one click re-sync). A group pill only shows when the group is visible with at least one active member, ranks with the standard Yappers scoring, and has its own shareable `/?board=slug` link. One person can sit in several groups, and removing someone from a group never touches the main board. Visible groups also appear in the live `llms.txt`, `sitemap.md`, and `sitemap.xml` (2026-08-16).
- Site branding settings at `/admin/settings`, behind the gear icon in the admin nav. Change the site title, community name, board name, header title, and logo in one pass; a save updates the header lockup, the board heading, the browser tab title, the share text, the join page, and the discovery files together. Every field defaults to the shipped Friends of Convex look, so an untouched deploy renders exactly as before, and Reset to defaults clears every override including an uploaded logo (2026-08-16).
- A fork toggle for the Convex mentions tab. Board settings in `/admin` gains "Show the Convex mentions tab"; unchecking it hides the pill for forks that do not track Convex mentions, and a direct `/?board=convex` link falls back to Yappers. The README documents the three fork tiers: rebrand from admin, hide the Convex tab, or remove the Convex parts in code (2026-08-16).
- A test suite. `npm run test` runs vitest with convex-test: one file proves a board past 100 active profiles is paged completely and score blind (the exact shape of the sync skip bug), the other pins the post counting and Convex mention rules with fixtures for replies, quote posts, reposts, long post text, expanded convex.dev links, and the "convexity" non-match. `npm run check` now runs the tests too. Refs [issue 2](https://github.com/waynesutton/friendsofconvex-yapper-leaderboard/issues/2) (2026-08-16).
- Avatar bio peek on the board. Hover, focus, or tap any profile photo and a small card opens with that person's X bio, follower count, and an Open on X link, so you can meet the person without leaving the ranking. The bio's @mentions and URLs are tappable, the card works with the keyboard (focus opens, Escape closes), only one card shows at a time, and empty bios degrade to name, handle, and the X link. The data was already on every public leaderboard row, so this adds no schema or sync changes (2026-08-16).
