...
- [x] Rows with any past successful sync keep their metrics and rank when a later sync fails. `hasSyncedMetrics` in `convex/profiles.ts` drives `compareYapperRows`; `hasMetrics` in `src/components/Leaderboard.tsx` drives the client sort, every metric cell, the freshness chip, and the Convex mode help states. `src/components/ProfilePeek.tsx` shows followers on the same rule.
- [x] New public `profiles.getSyncHealth` query (key configured, active / synced / failed / never synced counts, most common X error). The board notice now branches: missing key, X rejecting requests with the message, first sync pending, or "Showing the last synced numbers from <date>" when every row's latest sync failed. Partial failures stay quiet.
- [x] `syncBatchesFrom` in `convex/xSync.ts` halts at the first spend cap error (`isSpendCapError` in `convex/xSyncParsing.ts`), logs it, and returns `haltedReason`. `refreshAll` includes it in the result and `src/components/AdminPanel.tsx` Sync everyone says why it stopped. Untouched profiles keep their last good status.
- [x] Verified with `npx convex dev --once`, `npx tsc --noEmit`, `npx vitest run` (32 tests, 3 new), `npx eslint` on touched files, `npm run build`, and `npx convex run profiles:getSyncHealth` on dev. Production still needs `npx convex deploy` to pick this up.
...
- [x] Public board: muted rows render after an "In the group, off the ranking" divider with a dash for a rank and no medal, rank numbers skip them so the row below keeps its real position, and the client sort checks muted before every other comparison so no column sort or search moves them across the divider. `src/components/Leaderboard.tsx`, `src/globals.css`.
- [x] Mute and Unmute buttons plus a muted / retired status line on each group member row, and a Mute entry in How pills work. `src/components/GroupsPanel.tsx`.
- [x] Retire mode: optional `retiredAt` and `retiredNote` on `profiles`, a `profiles.setRetired` admin mutation returning the page path, and a `profiles.getRetired` public query. Retired people leave all three leaderboard paths, the group active member count, and the discovery files. Retiring twice keeps the original date and only replaces the note. `convex/schema.ts`, `convex/profiles.ts`, `convex/siteFiles.ts`, `convex/validators.ts`.
- [x] Admin UI: Retire opens an inline note field on that row (site design system, no browser prompts) and publishes on submit; retired rows show a retired status, a Champion page link, and Unretire. Retire is disabled for archived profiles. `src/components/AdminPanel.tsx`, `src/globals.css`.
- [x] New `/retired/:handle` champion page: racing stripe card in the gift pass style, the admin's note, retirement date, final posts / engagements / impressions / followers, Post on X, copy link, and a back-to-the-board line. Unknown or unretired handles get a short "nobody has been retired under that handle" card. Crawlers get per person meta tags from a `/retired/` HTTP route reusing the gift share rewrite pattern and the shipped OG image. `src/pages/RetiredPage.tsx`, `src/App.tsx`, `convex/sharePages.ts`, `convex/http.ts`, `src/globals.css`.
- [x] `/admin/docs` covers both: a mute paragraph in the Groups section and a new Retire mode section that spells out how retire differs from archive. `src/pages/AdminDocsPage.tsx`.
- [x] Verified with `npx tsc --noEmit`, `npx eslint`, `npx vitest run` (27 tests), `vite build`, and a clean `npx convex dev --once` push. Browser check of `/retired/nobodyhere` and the home board came back clean; the admin controls need a signed in admin session.
